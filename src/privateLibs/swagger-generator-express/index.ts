...
function getBodyParameters(
    ...
    describe: { tags?: Array<any>; common: any },
) {
...
function describeSwagger(routePath: string, requestModelPath: string, responseModelPath: any) {
    try {
        const rootPath = resolve(__dirname).split('dist')[0];
            ...
            if (file.includes('.js') && !file.includes('.js.')) {
                ...
                if (responseModelPath) {
                    ...
                    if (fs.existsSync(responseModelFullPath)) {
                        ...
                        console.log(
                            'Response model path does not exist responseModelFullPath->',
                            responseModelFullPath,
                        );
                    }
                }
